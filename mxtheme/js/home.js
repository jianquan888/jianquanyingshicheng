...
var MAC={
    ...
    'Verify': {
        'Init': function(){
            MAC.Verify.Focus();
            MAC.Verify.Click();
            $('body').on("click",'.search-verify-btn',function(){
                MAC.Verify.ShowVerifyPage();
            });
        },
        ...
        'ShowVerifyPage':function(){
            MAC.Pop.Show(maccms.path+'/index.php/search/show_puzzlevrf',function($r){
    ...
    'User':{
        ...
        'Init':function(){
            ...
            if(MAC.Cookie.Get('user_id') !=undefined && MAC.Cookie.Get('user_id')!=''){
                ...
                // $('.mac_user').removeClass('mac_user').removeClass("header-op-user").addClass("haslogin_user");
            ...
            if($(".user-headimg").length > 0){
                $("body").on("click",'.user-headimg',function(e){
                     MAC.Pop.Show(maccms.path+'/index.php/user/select_headimg',function($r){
                         $("#headImgBox>img").bind("click",function(e){
                             var userHeadImg = $(this).attr("src");
                            $.ajax({
                               type:'POST',
                               url:maccms.path + '/index.php/user/save_userHeadImg?user_portrait='+userHeadImg,
                               success:function($r){
                                   layer.msg($r.msg);
                                   if($r.code==1){
                                       ...
                                       location.reload();
                                       headImg = location.getItem("headImg");
    ...
    'webPageAdvert':function(){
        $('body').on('click','.webpage-advert',function(){
            var that=$(this);
            var target=that.children('span').html();
            window.location.href=target;
        });
    },
    'jumpAdvUrl':function(){
        var that=$(".index-banner-adv");
        var objChd=that.children("span.target-url");
        if(objChd){
            var targUrl=objChd.html();
